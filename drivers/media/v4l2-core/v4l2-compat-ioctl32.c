 ...
 * ioctls.
 */
#include <linux/compat.h> include <linux/module.h> include 
#<linux/videodev2.h> include <linux/v4l2-subdev.h> include 
#<media/v4l2-dev.h> include <media/v4l2-ioctl.h> define 
#convert_in_user(srcptr, dstptr) \
({ \
	typeof(*srcptr) val; \
							\
...
static int get_v4l2_window32(struct v4l2_window __user *kp, struct
		v4l2_window32 __user *up, void __user *aux_buf, int 
aux_space) {
	__u32 clipcount;
...
static int put_v4l2_window32(struct v4l2_window __user *kp, struct 
v4l2_window32 __user *up) {
...
static inline int get_v4l2_pix_format_mplane(struct v4l2_pix_format_mplane 
__user *kp,
...
static inline int put_v4l2_pix_format_mplane(struct v4l2_pix_format_mplane 
__user *kp,
				struct v4l2_pix_format_mplane __user *up) {
...
__user *kp, struct v4l2_sliced_vbi_format __user *up) {
...
static int __get_v4l2_format32(struct v4l2_format __user *kp, struct
		v4l2_format32 __user *up, void __user *aux_buf, int 
aux_space) {
	__u32 type;
...
static int get_v4l2_create32(struct v4l2_create_buffers __user *kp, struct
	...
	if (!access_ok(VERIFY_READ, up, sizeof(struct 
v4l2_create_buffers32)) ||
	    copy_in_user(kp, up, offsetof(struct v4l2_create_buffers32, 
format)))
		return -EFAULT;
	return __get_v4l2_format32(&kp->format, &up->format, aux_buf, 
aux_space);
}
static int __put_v4l2_format32(struct v4l2_format __user *kp, struct 
...
static int put_v4l2_format32(struct v4l2_format __user *kp, struct 
v4l2_format32 __user *up) {
	if (!access_ok(VERIFY_WRITE, up, sizeof(struct v4l2_format32)))
		return -EFAULT;
	return __put_v4l2_format32(kp, up);
}
static int put_v4l2_create32(struct v4l2_create_buffers __user *kp, struct 
v4l2_create_buffers32 __user *up) {
	if (!access_ok(VERIFY_WRITE, up, sizeof(struct 
v4l2_create_buffers32)) ||
	    copy_in_user(up, kp, offsetof(struct v4l2_create_buffers32, 
format)) ||
...
static int get_v4l2_standard32(struct v4l2_standard __user *kp, struct 
...
static int put_v4l2_standard32(struct v4l2_standard __user *kp, struct 
v4l2_standard32 __user *up) {
...
sizeof(kp->frameperiod)) ||
		...
		copy_in_user(up->reserved, kp->reserved, 4 * 
sizeof(__u32)))
			return -EFAULT;
	return 0;
}
...
static int get_v4l2_buffer32(struct v4l2_buffer __user *kp, struct
		v4l2_buffer32 __user *up, void __user *aux_buf, int 
aux_space) {
	__u32 type;
	__u32 length;
	enum v4l2_memory memory;
	...
	if (V4L2_TYPE_IS_OUTPUT(type))
		if (convert_in_user(&up->bytesused, &kp->bytesused) ||
			convert_in_user(&up->field, &kp->field) ||
			convert_in_user(&up->timestamp.tv_sec, 
&kp->timestamp.tv_sec) ||
	...
	if (V4L2_TYPE_IS_MULTIPLANAR(type)) {
		num_planes = length;
		if (num_planes == 0) {
			/* num_planes == 0 is legal, e.g. when userspace 
doesn't
...
static int put_v4l2_buffer32(struct v4l2_buffer __user *kp, struct 
v4l2_buffer32 __user *up) {
	__u32 type;
	__u32 length;
	enum v4l2_memory memory;
	struct v4l2_plane32 __user *uplane32;
...
&up->timestamp.tv_usec) ||
		copy_in_user(&up->timecode, &kp->timecode, sizeof(struct 
v4l2_timecode)) ||
...
**)&kp->m.planes)))
		...
		case V4L2_MEMORY_MMAP:
			...
			break;
		case V4L2_MEMORY_USERPTR:
			if (convert_in_user(&kp->m.userptr, 
&up->m.userptr))
				return -EFAULT;
			break;
		case V4L2_MEMORY_OVERLAY:
...
||
		...
		convert_in_user(&up->flags, &kp->flags) ||
		get_v4l2_pix_format((struct v4l2_pix_format *)&kp->fmt, 
(struct v4l2_pix_format *)&up->fmt))
			return -EFAULT;
	return 0;
}
...
||
		...
		convert_in_user(&kp->flags, &up->flags) ||
		put_v4l2_pix_format((struct v4l2_pix_format *)&kp->fmt, 
(struct v4l2_pix_format *)&up->fmt))
			return -EFAULT;
	return 0;
}
...
/* The 64-bit v4l2_input struct has extra padding at the end of the struct.
   Otherwise it is identical to the 32-bit version. */ static inline int 
get_v4l2_input32(struct v4l2_input __user *kp, struct v4l2_input32 __user 
*up) {
	if (copy_in_user(kp, up, sizeof(struct v4l2_input32)))
		return -EFAULT;
	return 0;
}
static inline int put_v4l2_input32(struct v4l2_input __user *kp, struct 
...
/* The following function really belong in v4l2-common, but that causes
   ...
   for now this will do. */ /* Return non-zero if this control is a pointer 
type. Currently only
   type STRING is a pointer type. */ static inline int ctrl_is_pointer(u32 
id) {
	switch (id) {
...
__user *up) {
	if (!access_ok(VERIFY_WRITE, up, sizeof(struct v4l2_event32)) ||
		...
		convert_in_user(&kp->timestamp.tv_sec, 
&up->timestamp.tv_sec) ||
		convert_in_user(&kp->timestamp.tv_nsec, 
&up->timestamp.tv_nsec) ||
		convert_in_user(&kp->id, &up->id) ||
		copy_in_user(up->reserved, kp->reserved, 8 * 
sizeof(__u32)))
			return -EFAULT;
	return 0;
}
...
#define VIDIOC_G_FMT32 _IOWR('V', 4, struct v4l2_format32) define 
#VIDIOC_S_FMT32 _IOWR('V', 5, struct v4l2_format32) define 
#VIDIOC_QUERYBUF32 _IOWR('V', 9, struct v4l2_buffer32) define 
#VIDIOC_G_FBUF32 _IOR ('V', 10, struct v4l2_framebuffer32) define 
#VIDIOC_S_FBUF32 _IOW ('V', 11, struct v4l2_framebuffer32) define 
#VIDIOC_QBUF32 _IOWR('V', 15, struct v4l2_buffer32) define VIDIOC_DQBUF32 
#_IOWR('V', 17, struct v4l2_buffer32) define VIDIOC_ENUMSTD32 _IOWR('V', 
#25, struct v4l2_standard32) define VIDIOC_ENUMINPUT32 _IOWR('V', 26, 
#struct v4l2_input32) define VIDIOC_G_EDID32 _IOWR('V', 40, struct 
#v4l2_edid32) define VIDIOC_S_EDID32 _IOWR('V', 41, struct v4l2_edid32) 
#define VIDIOC_TRY_FMT32 _IOWR('V', 64, struct v4l2_format32) define 
#VIDIOC_G_EXT_CTRLS32 _IOWR('V', 71, struct v4l2_ext_controls32) define 
#VIDIOC_S_EXT_CTRLS32 _IOWR('V', 72, struct v4l2_ext_controls32) define 
#VIDIOC_TRY_EXT_CTRLS32 _IOWR('V', 73, struct v4l2_ext_controls32) define 
#VIDIOC_DQEVENT32 _IOR ('V', 89, struct v4l2_event32) define 
#VIDIOC_CREATE_BUFS32 _IOWR('V', 92, struct v4l2_create_buffers32) define 
#VIDIOC_PREPARE_BUF32 _IOWR('V', 93, struct v4l2_buffer32) define 
#VIDIOC_OVERLAY32 _IOW ('V', 14, s32) define VIDIOC_STREAMON32 _IOW ('V', 
#18, s32) define VIDIOC_STREAMOFF32 _IOW ('V', 19, s32) define 
#VIDIOC_G_INPUT32 _IOR ('V', 38, s32) define VIDIOC_S_INPUT32 _IOWR('V', 
#39, s32) define VIDIOC_G_OUTPUT32 _IOR ('V', 46, s32) define 
#VIDIOC_S_OUTPUT32	_IOWR('V', 47, s32)
/*
 * Note that these macros contain return statements to avoid the need for 
the
...
#define ALLOC_AND_GET(bufsizefunc, getfunc, structname) \
	do { \
		...
		if (aux_space < 0) \
			return aux_space; \
		up_native = ALLOC_USER_SPACE(sizeof(struct structname) + 
aux_space); \
...
long arg) {
	...
	case VIDIOC_G_EDID:
	case VIDIOC_S_EDID:
		up_native = ALLOC_USER_SPACE(sizeof(struct 
v4l2_subdev_edid));
	...
	case VIDIOC_S_FMT:
	case VIDIOC_TRY_FMT:
		ALLOC_AND_GET(bufsize_v4l2_format32, get_v4l2_format32, 
v4l2_format);
		compatible_arg = 0;
		break;
	case VIDIOC_CREATE_BUFS:
		ALLOC_AND_GET(bufsize_v4l2_create32, get_v4l2_create32, 
v4l2_create_buffers);
		compatible_arg = 0;
		break;
	...
	case VIDIOC_QBUF:
	case VIDIOC_DQBUF:
		ALLOC_AND_GET(bufsize_v4l2_buffer32, get_v4l2_buffer32, 
v4l2_buffer);
		compatible_arg = 0;
		break;
	case VIDIOC_S_FBUF:
...
v4l2_framebuffer));
	...
	case VIDIOC_TRY_EXT_CTRLS:
		ALLOC_AND_GET(bufsize_v4l2_ext_controls32, 
get_v4l2_ext_controls32, v4l2_ext_controls);
		compatible_arg = 0;
		break;
	case VIDIOC_DQEVENT:
